...
void AProjectile::ApplyProjectileCollision()
{
	...
	// 1 Projectile 프리셋으로 리셋 (기본 응답/세팅을 항상 동일하게 시작)
	Collision->SetCollisionProfileName(TEXT("Projectile"));

	// // 오버랩 기반 - QueryOnly/OverlapEvents를 확실히 켜두기
	...
	// 2 Set된 OwnerCollisionChannel 무시 추가
	if (OwnerCollisionChannel != ECC_MAX)
	{
		Collision->SetCollisionResponseToChannel(OwnerCollisionChannel, ECR_Ignore);
	}
}
